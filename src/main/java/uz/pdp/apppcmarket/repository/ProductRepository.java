...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    @Query(value = "select * from product\n" +
     ...
     Page<Product> getProductsbyNameModelCharacterPrice(String name, String model, String price, Integer chId);

    @Query(value = "select * from product\n" +
            "join character c on product.id = c.product_id\n" +
            "join property p on c.id = p.character_id\n" +
            "where p.id in (?)", nativeQuery = true)
     Page<Product> getProductsbyPropertyList(List<Integer> propertyList);
}
